...
#        
 ...
 #       y=df.iloc[:,[9]].values   #chla 
        ...
        y=df.iloc[:,[8]].values   #cell counts
        
        
        #≥1.5×104 cells l−1 defined a detectable HAB.
        y2=np.copy(y)
        y2[:]=0
        y_idx=np.where(y>100000)
...
def train(csv_file, n_epochs=100):
   ...
   # inputSize = len(trainDataset.columns) -1
    ...
    plt.plot(outputs.detach().cpu().numpy(),labels.detach().cpu().numpy(),'o')
...
if __name__ == "__main__":
    ...
    csv_file = os.path.join("/home/zhichen/Documents/pytorch/sorted_same_day_data_v3.csv")
